...
export function TeamCard({
  ...
  return (
    <article className="group relative flex flex-col bg-card p-7 transition-colors duration-300 hover:bg-card-elevated md:p-9">
      <div
...
type TeamCardsRowProps = React.HTMLAttributes<HTMLDivElement> & {
  members?: TeamMember[];
};

export function TeamCardsRow({
  members = TEAM,
  className,
  style,
  ...rest
}: TeamCardsRowProps) {
  const total = members.length.toString().padStart(2, "0");
  return (
    <div
      {...rest}
      className={cn(
        "grid overflow-hidden rounded-lg border border-border bg-border md:grid-cols-3",
        className,
      )}
      style={{ gap: 1, ...style }}
    >
      {members.map((member) => (
        <TeamCard key={member.name} member={member} total={total} />
      ))}
    </div>
  );
}
